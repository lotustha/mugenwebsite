...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/categories").then((r) => r.json()).then((d) => setItems(Array.isArray(d) ? d : [])).finally(() => setLoading(false));
  }, []);

  const create = async () => {
    if (!newName.trim()) return;
    setError(""); setCreating(true);
    const res = await fetch("/api/admin/categories", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: newName.trim() }) });
  ...
  const save = async (id: string) => {
    ...
    const res = await fetch(`/api/admin/categories/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: editName.trim() }) });
    const data = await res.json();
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        <h1 className="font-headline text-2xl font-bold text-text-main">Categories</h1>
        <p className="font-body text-sm mt-0.5" style={{ color: "rgba(222,229,255,0.4)" }}>{items.length} categories</p>
      </div>

      {/* Create */}
      <div className="rounded-xl p-5 mb-6" style={{ background: "rgba(9,19,40,0.5)", border: "1px solid rgba(255,255,255,0.07)" }}>
        <p className="font-headline text-sm font-semibold text-text-main mb-3">Add New Category</p>
      ...
      {loading ? (
        ...
        <p className="font-body text-text-main/40 text-center py-12">No categories yet.</p>
      ) : (
            ...
            <div key={cat.id} className="flex items-center gap-3 px-4 py-3 rounded-xl"
              ...
              {editId === cat.id ? (
                <>
                  ...
                  <button type="button" onClick={() => save(cat.id)}
                    className="px-3 py-1.5 rounded-lg font-headline text-xs font-semibold cursor-pointer transition-all shrink-0"
                    style={{ background: "rgba(186,158,255,0.15)", color: "#ba9eff" }}>Save</button>
                  <button type="button" onClick={() => setEditId(null)}
                    className="px-3 py-1.5 rounded-lg font-body text-xs cursor-pointer transition-all shrink-0"
                    style={{ color: "rgba(222,229,255,0.4)" }}>Cancel</button>
                </>
              ) : (
                <>
                  <div className="flex-1 min-w-0">
                    <p className="font-body text-text-main text-sm">{cat.name}</p>
                  ...
                  <button type="button" onClick={() => remove(cat.id)}
